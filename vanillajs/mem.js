...
(function (sandbox) {
    ...
    require('jalangi/src/js/iidToLocation');
    ...
    function ObjectIndex() {
        ...
        function addToODBase(iid, objectId, isFrame, obj) {
            var tmp;
                tmp = odbase[iid];
                if (!tmp) {
                    tmp = odbase[iid] = {};
                ...
                if (!objects_with_last_use[obj]) {
                    tmp.currentStaleCount++;
                }
//            if (iid == 66881) {
//                console.log("^"+tmp.currentStaleCount+":"+tmp.currentAliveCount+":"+obj);
//            }
        }
        ...
        function annotateObject(iid, obj, isFrame) {
            ...
            if (sobj) {
                ...
                if (sobj.creationIndex === undefined) {
                    ...
                    addToODBase(getAllocIID(sobj.creationIndex), sobj.objectId, isFrame, obj);
                }
            }
        }
        ...
        function accessObject(obj, unreachable) {
            ...
            if (sobj && sobj.creationIndex) {
                ...
                if (!unreachable) {
                    infoObj.currentStaleCount++;
//                    if (iid3 == 66881) {
//                        console.log("A"+infoObj.currentStaleCount+":"+infoObj.currentAliveCount+":"+obj);
//                    }
                }
                if (unreachable) {
                    ...
                    infoObj.currentStaleCount--;
//                    if (iid3 == 66881) {
//                        console.log("V"+infoObj.currentStaleCount+":"+infoObj.currentAliveCount+":"+obj);
//                    }
                }
            }
            if (unreachable) {
            ...
            if (callStackDepth === 0) {
                for (var iid in odbase) {
                    if (odbase.hasOwnProperty(iid)) {
                        var tmp = odbase[iid];
//                        if (iid == 66881) {
//                            console.log(tmp.currentStaleCount+":"+tmp.currentAliveCount);
//                        }
//
                        if (tmp.isIncreasing === undefined  && tmp.emptyStackCount > 0) {
                            if (tmp.currentStaleCount > tmp.oldStaleCount) {
                                tmp.isIncreasing = 1;
                            } else if (tmp.currentStaleCount < tmp.oldStaleCount) {
                                tmp.isIncreasing = -1;
                            } else {
                                tmp.isIncreasing = 0;
                            }
                        ...
                        tmp.oldStaleCount = tmp.currentStaleCount;
        ...
        this.endExecution = function (printEscapeTree) {
            ...
            for (var x in tmp) {
                if (tmp.hasOwnProperty(x)) {
                    ...
                    data.isLeakingDefinitely = (odbasei.isIncreasing > 1);
                    data.leakIntensity = odbasei.isIncreasing;
...
//                    console.log(odbasei.xysum+" "+odbasei.xsquaresum+" "+odbasei.ysum+" "+odbasei.cumulativeAliveCount+" "+odbasei.emptyStackCount);
                    ...
                    console.log(data.total + " "+(data.isFrame ? "call frame(s)" : "object(s)/function(s)/array(s)") +
                        ...
                        (data.isLeakingDefinitely ? ("\n    is definitely leaking ("+data.leakIntensity+")") : "") +
    ...
    function getobjIdToNewIID(traceFile) {
        ...
        while (traceFh.hasNextLine()) {
            ...
            if (record[0] === EVENT_IDS.LAST_USE) {
                ...
                objects_with_last_use[record[1]] = true;
            }
...
//                    break;
    ...
    var FileLineReader = require('jalangi/src/js/utils/FileLineReader');
