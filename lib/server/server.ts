...
function initOutputTarget(): void {
    if (args.outputFile) {
        ...
        javaProc.on("exit", () => {
            console.timeEnd("lifetime finish");
            console.log("done");
            process.exit(0);
//            showGui();
        });
...
function record() {
    if (args.clientMode) {
        var exeSync = require("exec-sync");
        var WebSocket = require('ws');
        var log;
        // sdb root on
        log = exeSync('sdb root on');
        // kill previous tinyServer
        log = exeSync("sdb shell ps aux | grep tinyweb | grep -v grep | awk '{print $2}' | xargs sdb shell kill -9 ");
        //log = exeSync("sh -c sdb shell ps aux | grep tinyweb | grep -v grep | awk '{print $2}' | xargs sdb shell kill -9");
        console.log(log);
//		cp.spawn("sdb", "shell ps aux | grep tinyweb | grep -v grep | awk '{print $2}' | xargs sdb shell kill -9");
        // start tinyServer
        console.log("start the tiny server");
        exeSync('sdb shell "env LD_LIBRARY_PATH=/opt/home/developer PATH=/opt/home/developer:$PATH tinyweb -ssl_certificate /opt/home/developer/server.pem -document_root /opt/usr/media/tct/ -listening_ports 80,8080,8081,8082,8083,8443s" &');
        var sl = require('sleep');
        sl.sleep(3);
        // sdb port forwarding
        exeSync('sdb forward tcp:8082 tcp:8082');

        console.log("sdb forward");
        ...
        connection.on('open', function(){
            ...
            var runProc = cp.spawn('web-debug', ['-i', getApplicationID()]);
